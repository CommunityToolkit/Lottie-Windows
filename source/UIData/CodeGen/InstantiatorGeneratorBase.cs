    ...
    abstract class InstantiatorGeneratorBase
    {
            ...
            float height)
        {
            ...
            var info = new CodeGenInfo(
                ...
                usesLoadedImageSurface: _nodes.Where(n => n.UsesLoadedImageSurface).Any());
            ...
            foreach (var node in _nodes)
            {
                if (node.UsesLoadedImageSurface)
                {
                    var loadedImageSurface = (Wmd.LoadedImageSurface)node.Object;
                    builder.WriteLine($"static readonly byte[] {node.LoadedImageSurfaceBytesFieldName} = new byte[]");
                    builder.OpenScope();
            ...
            if (!node.Inlined)
            {
                switch (node.Type)
                {
                    ...
                    case Graph.NodeType.LoadedImageSurface:
                        GenerateLoadedImageSurfaceFactory(builder, (Wmd.LoadedImageSurface)node.Object, node);
                        return;
                    default:
                        throw new InvalidOperationException();
                }
        ...
        bool GenerateLoadedImageSurfaceFactory(CodeBuilder builder, Wmd.LoadedImageSurface obj, ObjectData node)
        {
            var fieldName = node.LoadedImageSurfaceBytesFieldName;

            WriteObjectFactoryStart(builder, node);
            WriteLoadedImageSurfaceFactory(builder, obj, fieldName);
            WriteObjectFactoryEnd(builder);
            return true;
        }
        ...
        protected sealed class CodeGenInfo
        {
                ...
                bool usesLoadedImageSurface)
            {
                ...
                UsesLoadedImageSurface = usesLoadedImageSurface;
            }
            ...
            /// <summary>
            /// Gets a value indicating whether the composition depends the Windows.UI.Xaml.Media namespace.
            /// </summary>
            public bool UsesLoadedImageSurface { get; }
        ...
        sealed class ObjectData : Graph.Node<ObjectData>
        {
            ...
            // TODO: Set to indicate that the node relies on LoadedImageSurface in Windows.UI.Xaml.Media namespace
            internal bool UsesLoadedImageSurface => Object is Wmd.LoadedImageSurface;
