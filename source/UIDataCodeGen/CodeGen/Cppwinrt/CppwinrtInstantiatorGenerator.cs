...
namespace Microsoft.Toolkit.Uwp.UI.Lottie.UIData.CodeGen.Cppwinrt
{
    ...
    sealed class CppwinrtInstantiatorGenerator : InstantiatorGeneratorBase
    {
        ...
        string GenerateIdlText()
        {
            ...
            builder.WriteLine($"runtimeclass {SourceInfo.ClassName}");
            builder.Indent();
            builder.WriteLine($": [default] {SourceInfo.InterfaceType.NormalizedQualifiedName}Source");
            builder.UnIndent();
            builder.OpenScope();
            builder.WriteLine($"{SourceInfo.ClassName}();");
        ...
        string GenerateHeaderText()
        {
            ...
            builder.Postamble.WriteCodeBuilder(WriteFactoryImplementationHeaderText());

            return builder.ToString();
        }

        CodeBuilder WriteFactoryImplementationHeaderText()
        {
            var builder = new CodeBuilder();
            builder.WriteLine("namespace factory_implementation");
            builder.OpenScope();
            builder.WriteLine($"struct {SourceInfo.ClassName} : {SourceInfo.ClassName}T<{SourceInfo.ClassName}, implementation::{SourceInfo.ClassName}>");
            builder.OpenScope();
            builder.CloseCppTypeScope();
            builder.CloseScope();
            return builder;
        ...
        void WriteIDynamicAnimatedVisualSourceHeaderText(HeaderBuilder builder)
        {
            ...
            // TODO: support INotifyPropertyChange in cppwinrt
            builder.Preamble.WriteLine($"class {_sourceClassName}");
            builder.Preamble.Indent();
            builder.Preamble.WriteLine($": public {_sourceClassName}T<{_sourceClassName}>");
            ...
            builder.Preamble.WriteLine();
            builder.Preamble.WriteLine("event Windows::Foundation::TypedEventHandler<IDynamicAnimatedVisualSource, winrt::Windows::Foundation::IInspectable> AnimatedVisualInvalidated");
            builder.Preamble.OpenScope();
            builder.Preamble.WriteLine("Windows::Foundation::EventRegistrationToken add(Windows::Foundation::TypedEventHandler<IDynamicAnimatedVisualSource, winrt::Windows::Foundation::IInspectable> value)");
            builder.Preamble.OpenScope();
            ...
            builder.Preamble.WriteLine();
            builder.Preamble.WriteLine($"winrt::{_animatedVisualTypeName} TryCreateAnimatedVisual(");
            builder.Preamble.Indent();
            builder.Preamble.WriteLine($"{_wuc}::Compositor compositor,");
            builder.Preamble.WriteLine($"winrt::Windows::Foundation::IInspectable* diagnostics);");
            builder.Preamble.UnIndent();
            builder.Preamble.WriteLine();
            builder.Preamble.WriteLine($"event PropertyChangedEventHandler PropertyChanged;");
            ...
            builder.Class.Private.WriteLine($"const int c_loadedImageSurfaceCount = {SourceInfo.LoadedImageSurfaces.Distinct().Count()};");
            builder.Class.Private.WriteLine("double m_imageSuccessfulLoadingProgress{};");
            builder.Class.Private.WriteLine("int m_loadCompleteEventCount{};");
            builder.Class.Private.WriteLine("bool m_isAnimatedVisualSourceDynamic{};");
            builder.Class.Private.WriteLine("bool m_isImageLoadingCompleted{}");
            builder.Class.Private.WriteLine("bool m_isTryCreateAnimatedVisualCalled{}");
            builder.Class.Private.WriteLine("bool m_isImageLoadingStarted{}");
            builder.Class.Private.WriteLine("event Windows::Foundation::TypedEventHandler<IDynamicAnimatedVisualSource, winrt::Windows::Foundation::IInspectable> m_InternalHandler;");

            foreach (var n in SourceInfo.LoadedImageSurfaces)
            {
                builder.Class.Private.WriteLine($"{n.TypeName} {n.FieldName}{{}};");
            }

            builder.Class.Private.WriteLine("void EnsureImageLoadingStarted();");
            builder.Class.Private.WriteLine("void HandleLoadCompleted(LoadedImageSurface sender, LoadedImageSourceLoadCompletedEventArgs e);");
            builder.Class.Private.WriteLine("void RaiseAnimatedVisualInvalidatedEvent(IDynamicAnimatedVisualSource sender, winrt::Windows::Foundation::IInspectable object)");
            builder.Class.Private.OpenScope();
            builder.Class.Private.OpenScope();
            builder.Class.Private.WriteLine("m_InternalHandler::raise(sender, object);");
            builder.Class.Private.CloseScope();
            builder.Class.Private.CloseScope();
            builder.Class.Private.CloseScope();
        }
            ...
            IAnimatedVisualInfo info)
        {
            ...
            {
                // D2D factory field.
                builder.WriteLine("winrt::com_ptr<ID2D1Factory> _d2dFactory;");
            }
        }
        ...
        protected override void WriteCanvasGeometryCombinationFactory(CodeBuilder builder, CanvasGeometry.Combination obj, string typeName, string fieldName)
        {
            ...
            builder.WriteLine("winrt::check_hresult(_d2dFactory->CreatePathGeometry(&path));");
            builder.WriteLine("ComPtr<ID2D1GeometrySink> sink;");
            builder.WriteLine("winrt::check_hresult(path->Open(&sink));");
        ...
        protected override void WriteCanvasGeometryTransformedGeometryFactory(CodeBuilder builder, CanvasGeometry.TransformedGeometry obj, string typeName, string fieldName)
        {
            ...
            builder.WriteLine("winrt::check_hresult(_d2dFactory->CreateTransformedGeometry(geoA, transformMatrix, &transformed));");
            builder.WriteLine("geoA->Release();");
        ...
        {
            builder.WriteLine($"bool {_s.Namespace(SourceInfo.Namespace)}::{_sourceClassName}::IsAnimatedVisualSourceDynamic::get()");
            builder.OpenScope();
            builder.WriteLine("return m_isAnimatedVisualSourceDynamic;");
            builder.CloseScope();
            builder.WriteLine();
            builder.WriteLine($"void {_s.Namespace(SourceInfo.Namespace)}::{_sourceClassName}::IsAnimatedVisualSourceDynamic::set(bool isAnimatedVisualSourceDynamic)");
            builder.OpenScope();
        ...
        protected override void WriteCanvasGeometryPathFactory(CodeBuilder builder, CanvasGeometry.Path obj, string typeName, string fieldName)
        {
            ...
            builder.WriteLine("winrt::com_ptr<ID2D1GeometrySink> sink;");
            ...
            builder.WriteLine("winrt::check_hresult(sink->Close());");
            builder.WriteLine("result = winrt::make_self<CanvasGeometry>(path);");
        }
        ...
        protected override void WriteCanvasGeometryGroupFactory(CodeBuilder builder, CanvasGeometry.Group obj, string typeName, string fieldName)
        {
            ...
            builder.CloseCppTypeScope();
            builder.WriteLine($"{typeName} result;");
            builder.WriteLine("winrt::com_ptr<ID2D1GeometryGroup> group;");
            ...
            builder.UnIndent();
            builder.WriteLine($"result = {FieldAssignment(fieldName)}winrt::make_self<CanvasGeometry>(group);");
        }
...
@"class CanvasGeometry final : public winrt::implements<CanvasGeometry,
    ...
    {
        winrt::com_ptr<ID2D1Geometry> _geometry;
